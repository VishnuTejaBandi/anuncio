...
export type AnuncioItemCloseEventPayload = {
    closedItem: ImageItem | VideoItem;
};
export type AnuncioItemStartEventPayload = {
    startedItem: ImageItem | VideoItem;
};
export type AnuncioCloseEventPayload = {
    maxProgressMap: Record<ImageItem["id"] | VideoItem["id"], {
        percentage: number;
        value: number;
    }>;
};
...
export type AnuncioPayloadByType<T extends AnuncioEventType> = Parameters<AnuncioEventListenerMap[T]>[0] extends CustomEvent<infer P> ? P : undefined;
/**
 * @private
 */
